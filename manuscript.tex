...
All adopted a similar headline discount rate (\approx0.05), but diverged in their treatment of equality ($\sigma$), growth ($g$), and time preference ($\delta$).\\

\begin{itemize}
	\item Cline assumed strong preference for equality ($\sigma$=1.5) and no time preference ($\delta$=0), producing a high savings rate ($S$\approx0.67) and advocating significant investment in mitigation.
	\item Nordhaus adopted moderate values ($\sigma$=1,$\delta$=0.03), implying less concern for future generations ($S$\approx0.4).
	\item Stern combined low time preference ($\delta$=0.001) with high growth ($g$=0.049), producing an almost maximum savings rate ($S$\approx0.98) and calling for urgent action.
\end{itemize}
...
Point estimates for the Commonwealth Cost of Carbon in current prices are \approx2020\$140tCO2e, up from \approx2000\$65tCO2e.
...
Between 2000 and 2020, this cost rose from \approx2000\$65 to \approx2020\$140 per tonne of CO2e, revealing both a decline in ecosystem performance and a rising social value for emission reduction.
